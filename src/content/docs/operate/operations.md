...
bearer token, because the breakdown names your public base URL, your configured
Apps, and which parts are misconfigured, which is not something an unguarded
endpoint should hand to anyone who asks. An unauthenticated caller
...
learn to ignore. It still notes that a pipeline file *could* ask for one, since
that file lives in a commit the server has not seen yet.
...
when a job was killed mid-run by a `docker kill`, an OOM or a redeploy, leaving
a row with no worker behind it. That gap is the single most useful signal that a
...
what your existing configuration does, not that the schema stands still, which
is why the backup step above is not optional on a minor either.
